...
group_revision[msm8953]=LA.UM.10.6.2.r1-01900-89xx.0
group_revision[sdm660]=LA.UM.10.2.1.r1-03800-sdm660.0
...
group=${1:-sdm660} # Default just for testing
...
for repo in $(repo list -p -g $group); do
	...
	git -C $repo pull --no-commit --log $(cat $repo/.gitupstream) ${revision} && git commit --no-edit
done
