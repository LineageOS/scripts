...
def main():
    ...
    aidl_interface = package.get_versioned_package(version)

    aidl_object = parser.import_aidl(args.fqname, version)

    assert aidl_object is not None, f"{args.fqname} not found in {aidl_interface}"

    assert isinstance(aidl_object, Interface), f"{args.fqname} is not an interface"

    formatter = get_formatter(args.backend)

    formatter.dump_to_folder(package, aidl_interface, aidl_object, args.out_dir)
