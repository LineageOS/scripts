...
AIDL_TO_CPP_TYPE: Dict[PrimitiveType, Union[str, None]] = {
...
#include <{aidl_path}/Bn{class_name}.h>
...
using {aidl_namespace}::{class_name};

int main() {{
    android::ProcessState::self()->setThreadPoolMaxThreadCount(0);
    android::sp<{class_name}> {class_name_lower} = android::sp<{class_name}>::make();

    const android::String16 instance = {class_name}::descriptor + android::String16("/default");
    android::status_t status = android::defaultServiceManager()->addService(instance, {class_name_lower});
...
class CPPFormatter(Formatter):
    ...
    def format_method(
        ...
        if class_name is None:
            return f"::android::binder::Status {method.name}({', '.join(method_arguments)})"
        else:
            return f"::android::binder::Status {class_name}::{method.name}({', '.join(method_arguments)})"
    ...
    def format_data_type(
        ...
        if isinstance(data_type.data_type, DataType):
            ...
            result = f"::{data_type.data_type.replace('.', '::')}"
