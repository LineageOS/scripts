...
class CPPFormatter(Formatter):
    ...
    def _dump_to_folder(
        ...
        methods_definitions_strs: List[str] = []
        for method in interface.methods:
            method_return_type = cls.format_data_type(method.return_type)

            method_arguments = [
                f"{cls.format_data_type(arg.data_type)} {arg.name}"
                for arg in method.arguments
            ]
            if method.return_type.data_type != PrimitiveType.VOID:
                method_arguments.append(f"{method_return_type}* _aidl_return")

            methods_definitions_strs.append(f"::ndk::ScopedAStatus {class_name}::{method.name}({', '.join(method_arguments)})")
        ...
        methods_definitions = "\n\n".join([
            ...
            for method in methods_definitions_strs
        ])
