...
class CPPFormatter(Formatter):
    ...
    def _dump_to_folder(
        ...
        #(folder / f"{class_name}.cpp").write_text(MAIN_CPP_TEMPLATE.format(**template_kwargs))
        #(folder / f"{class_name}.h").write_text(MAIN_H_TEMPLATE.format(**template_kwargs))
    ...
    def format_data_type(cls, data_type: DataType) -> str:
        ...
        if data_type.data_type is DataType:
            result = cls.format_data_type(data_type.data_type)
        elif data_type.data_type is PrimitiveType:
            result = AIDL_TO_CPP_TYPE[data_type.data_type]
        elif data_type.data_type is str:
