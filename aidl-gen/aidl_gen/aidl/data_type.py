...
class DataType:
    ...
    def __init__(
        ...
        assert (self.array_size is None) == (self.collection_type is None or self.collection_type is CollectionType.ARRAY), \
    ...
    def from_aidl(cls, data_type: str) -> "DataType":
        ...
        # Check syntax
        assert data_type.count("[") == data_type.count("]"), f"Invalid syntax for data type {data_type}"
        assert data_type.count("<") == data_type.count(">"), f"Invalid syntax for data type {data_type}"
        ...
        # Should be a non-primitive non-collection type, let's hope it's a fqname
