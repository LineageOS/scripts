...
def process_list(path: str):
    devices: dict[str, tuple[list[int], list[int]]] = {}

    if not os.path.exists(path):
        return devices

    with open(path, 'r', encoding='utf-8') as f:
        dev_config = re.compile(
        ...
        for lineno, line in enumerate(f, start=1):
            ...
            match = dev_config.match(line)
            if not match:
                die(f'Invalid line in {path}:{lineno}: {line}')

            device = match.group('device')
            esim = [int(x) for x in match.group('esim').split(',') if x]
            psim = [int(x) for x in match.group('psim').split(',') if x]
...
def extract_from_strings(apk_path: Path):
    with TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            die(
                f'Failed to parse sim_slot_mappings_json: {exc}\n'
                f'Snippet: {snippet}'
            )
...
def generate_list(apk_path: str, devices_path: str) -> None:
    extracted = extract_from_strings(Path(apk_path))
    ...
    with open(devices_path, 'w', encoding='utf-8') as f:
        ...
        for dev in sorted(merged):
            ...
            esim = ','.join(str(x) for x in sorted(esim))
            psim = ','.join(str(x) for x in sorted(psim))
            f.write(f'{dev}[{esim}][{psim}]\n')

    color_print(f'Updated {devices_path}', color=Color.GREEN)


def generate_resources(values_dir: Path, devices) -> None:
    groups: dict[tuple[tuple[int, ...], tuple[int, ...]], list[str]] = (
        defaultdict(list)
    )
