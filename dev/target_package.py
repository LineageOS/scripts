...
def get_manifests_resources(module: Dict, base_manifest=False):
    manifests = []

    manifest = None
    if base_manifest:
        manifest = module.get('manifest', None)

    if manifest is not None:
        manifests.append(manifest)

    additional_manifests = module.get('additional_manifests', [])
    manifests.extend(additional_manifests)

    resources = module.get('resource_dirs', [])
...
def extend_with_defaults(
    ...
    for default_name in defaults:
        ...
        base = merge_dicts(base, defaults_module)

    if not base:
        return module

    return merge_dicts(base, module)
...
def get_app_resources(app_module: Dict):
...
@functools.cache
def map_packages_paths():
    package_path_map: Dict[str, Tuple[str, str]] = {}
    ...
    for parent_path in PACKAGE_LOCATIONS:
        ...
        for android_bp_path in get_files_with_name(
            ...
            for app_module in android_apps_map.values():
                ...
                resource_dirs = get_app_resources(app_module)
                ...
                package_path_map[package_name] = (
                    android_bp_dir_path,
                    get_existing_paths_rel(
                        android_bp_dir_path,
                        resource_dirs,
                    ),
                )
...
def get_target_package_path(target_package: str):
    package_path_map = map_packages_paths()
