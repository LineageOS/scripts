...
def update_certificates():
    ...
    parser.add_argument(
        'target',
        action='store',
        help='Path to android root directory in which to update certificates',
    )
    parser.add_argument(
        'source',
        action='store',
    ...
    name_cert_map: Dict[str, str] = {}
    for mac_permission_xml in find_glob(args.source, MAC_PERMISSIONS_PATTERN):
    ...
    for mac_permission_xml in find_glob(args.target, MAC_PERMISSIONS_PATTERN):
        read_mac_permission_xml(mac_permission_xml, name_token_map)

    token_path_map: Dict[str, str] = {}
    for keys_conf in find_glob(args.target, 'keys.conf'):
    ...
    for name, cert in name_cert_map.items():
        ...
        full_cert_path = Path(args.target, cert_path)
