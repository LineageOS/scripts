...
def _read_prefix_before_tag(xml_path: str, tag: str) -> Optional[bytes]:
    """
    Return raw bytes from start of file up to the first occurrence of '<tag'.
    This preserves XML declaration, newlines, comment blocks, indentation, etc.
    """
...
def write_manifest(
    ...
    # Build the manifest body (starting at <manifest ...>)
    body_lines: list[str] = []
    ...
    with open(output_path, 'wb') as o:
        if prefix is not None:
            # Prefix already includes XML declaration + comment(s) + exact whitespace.
            o.write(prefix)
        else:
            # Keep existing behavior: XML_COMMENT likely includes the leading newline before <!--
