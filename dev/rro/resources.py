...
def parse_xml_resource(
    ...
    for node in root:
        if isinstance(node, etree._Comment):
            ...
            if (
                not isinstance(last_node, etree._Comment)
        ...
        resource = Resource(index, xml_rel_path, keys, node, comments)
        index += 1
...
def is_referenced_resource_element(
    ...
    if element.text.strip() == reference_name:
        return True
...
def get_referencing_resource(
    ...
    reference_name = f'@{resource.keys[0]}/{resource.keys[1]}'
...
def group_overlay_resources_rel_path(
    ...
    wrong_type_resources: Set[Resource] = set()
    identical_resources: List[Resource] = []

    for resource in overlay_resources.values():
        package_resource = package_resources.get(resource.keys)
        if package_resource is None:
            if is_wrong_type_resource(resource, package_resources):
                wrong_type_resources.add(resource)
                continue
