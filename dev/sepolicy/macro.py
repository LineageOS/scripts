...
def macro_recursive_used_macros(
    ...
    # Rules are one-off, so they don't need to be tracked
...
"""
    ...
    # Create patterns for no-arity macros
    ...
    for macro in chain(processed_macros.values(), processed_rules):
        for no_arity_macro in no_arity_macros:
            assert no_arity_macro.name is not None
    ...
    for macro in chain(processed_macros.values(), processed_rules):
        if not macro.used_macros and not macro.used_variables:
    ...
    for macro in chain(processed_macros.values(), processed_rules):
        ...
        # The dummy call for a rule is its text
        if macro.name is None:
            dummy_call = macro.text
        else:
...
def resolve_macro_paths(
    ...
    for macro_path in macro_paths:
        ...
        for file_path in SEPOLICY_FILES:
            ...
            if not fp.exists():
                fp = Path(macro_path, 'public', file_path)
...
def unpack_ioctl(part: str):
    # TODO: unify with source rule parser
    if '-' not in part:
