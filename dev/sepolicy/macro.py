...
is_flag_enabled_regex = re.compile(
    r'\b(?:is_flag_(enabled|disabled))\s*\(\s*([^\s,)]+)',
)
...
"""
    ...
    for macro in chain(processed_macros.values(), processed_rules):
        ...
        try:
            ...
            color_print(
                f'Failed to find used macros for macro {macro.name}, with body: {macro.body}: {e}',
                color=Color.RED,
            )
            continue
...
def resolve_macro_paths(
    ...
    for macro_path in macro_paths:
        if macro_path.is_file():
            ...
            macro_file_paths.append(macro_path)
            continue
        ...
        for file_name in SEPOLICY_FILES:
            for file_prefix in SEPOLICY_FILES_PREFIXES:
                ...
                macro_file_paths.append(file_path)
...
def parse_ioctls(ioctls: List[Tuple[str, str]]):
