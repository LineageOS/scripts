...
def resolve_contexts_paths(
    ...
    contexts_file_paths: Dict[str, List[Path]] = {}

    def add_contexts_file_path(fp: Path):
        if not fp.is_file():
            return

        for contexts_name in CONTEXTS_NAMES:
            if partition_name is not None:
                file_name = f'{partition_name}_{contexts_name}'
            else:
                file_name = contexts_name

            if fp.name != file_name:
                continue

            if contexts_name not in contexts_file_paths:
                contexts_file_paths[contexts_name] = []

            contexts_file_paths[contexts_name].append(fp)
            if verbose:
...
def split_contexts_text(contexts_file_paths: Dict[str, List[Path]]):
    contexts_texts: Dict[str, List[str]] = {}

    for contexts_name, file_paths in contexts_file_paths.items():
        contexts_texts[contexts_name] = []
        ...
        for file_path in file_paths:
            ...
            for rule in split_rules(
                ...
                contexts_texts[contexts_name].append(rule)
...
def expand_contexts_texts(
    ...
    for contexts_name, context_texts in contexts_texts.items():
        contexts[contexts_name] = expand_context_texts(
            context_texts,
...
def parse_contexts_texts(contexts_texts: Dict[str, List[str]]):
    ...
    for contexts_name, context_texts in contexts_texts.items():
        ...
        if contexts_name == GENFS_CONTEXTS_NAME:
            for context_text in context_texts:
        ...
        for context_text in context_texts:
            ...
            contexts[contexts_name].append(tuple(context_parts))
...
def remove_source_contexts(
    contexts: Dict[str, List[Tuple[str, ...]]],
    source_contexts: Dict[str, List[Tuple[str, ...]]],
):
    new_contexts: Dict[str, List[Tuple[str, ...]]] = {}

    for contexts_name, contexts_rules in contexts.items():
        if contexts_name not in source_contexts:
            continue

        source_contexts_rules_set = set(source_contexts[contexts_name])
        ...
        color_print(
            f'Removed {removed_rules} {contexts_name} source rules',
            color=Color.GREEN,
        )

        new_contexts[contexts_name] = new_contexts_rules
...
def remove_source_genfs_rules(
    ...
    color_print(
        f'Removed {removed_rules} {GENFS_CONTEXTS_NAME} source rules',
        color=Color.GREEN,
    )
...
def output_contexts(
    ...
    for contexts_name, contexts_rules in contexts.items():
        ...
        output_contexts_path = output_dir / contexts_name
...
def output_genfs_contexts(genfs_rules: List[Rule], output_dir: Path):
    ...
    output_path = output_dir / GENFS_CONTEXTS_NAME
