...
def generate_rro_main():
    ...
    def is_overlay_excluded(overlay: Overlay):
        if overlay.package in exclude_overlays:
            if args.verbose:
                color_print(
        ...
        if overlay.target_package in exclude_packages:
            if args.verbose:
                color_print(
                    f'{overlay.package}: Excluded by {overlay.target_package}',
                    color=Color.GREEN,
                )
            return True
    ...
    for apk_data in apks_data:
        ...
        fixup_overlay_target_package(overlay)
