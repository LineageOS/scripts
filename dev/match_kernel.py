...
def init_repo(config: Config, project_name: str, remote_url: str):
    if config.mirror is None:
        project_url = f'{remote_url}{project_name}'
    else:
        project_url = f'file://{config.mirror}/{project_name}.git'
...
def fetch_remote():
    ...
    run_cmd(['git', 'fetch', REMOTE_NAME], capture=False)
    run_cmd(['git', 'fetch', '--tags', REMOTE_NAME], capture=False)
...
def list_matching_branches_tags(match_patterns: List[str]):
    proc = subprocess.Popen(
        [
            'git',
            'for-each-ref',
            # Sort descending order of creation date
            '--sort=-creatordate',
            '--format=%(refname)',
            *match_patterns,
        ],
...
def match_refs(refs: Iterable[str]):
    best_ref = None
    best_score = None
    for ref in refs:
