...
from sepolicy.classmap import SELINUX_INCLUDE_PATH, Classmap
...
def get_macros_path(version: Optional[str]):
...
def get_selinux_dir_policy(selinux_dir: str):
    ...
    if partition_name == 'vendor':
        ...
        platform_policy_path = Path(dump_root, 'system/etc/selinux/plat_sepolicy.cil')
        assert platform_policy_path.exists()
...
def decompile_cil():
    ...
    # TODO: allow passing kernel version and download classmap.h
    parser.add_argument(
        '-k',
        '--kernel',
        action='store',
    ...
    if not macros:
        macros_dir_path = str(get_macros_path(version))
        macros.append(macros_dir_path)
    ...
    macro_file_paths = resolve_macro_paths(macros)
