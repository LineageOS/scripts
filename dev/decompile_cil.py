...
def decompile_one_cil(
    ...
    fn = partial(
        ...
        decompiled_used_types=decompiled_used_types,
...
def decompile_cil():
    ...
    macro_paths = Path(args.macros) if args.macros is not None else None
    rule_paths = Path(args.rules) if args.rules is not None else None
    extra_macro_paths: List[Path] = [Path(s) for s in args.extra_macros]
    extra_rule_paths: List[Path] = [Path(s) for s in args.extra_rules]
    ...
    if not macro_paths:
        macro_paths = get_macros_path(version, args.current)
    ...
    color_print(f'Leftover rules: {count_decompiled_rules()}', color=Color.GREEN)
