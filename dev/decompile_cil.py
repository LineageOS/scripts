...
def decompile_cil():
    ...
    macros_path = Path(args.macros) if args.macros is not None else None
    rules_path = Path(args.rules) if args.rules is not None else None
    extra_macros_paths: List[Path] = [Path(s) for s in args.extra_macros]
    extra_rules_paths: List[Path] = [Path(s) for s in args.extra_rules]
    ...
    if not macros_path:
        macros_path = get_macros_path(version, args.current)

    if not rules_path:
        rules_path = macros_path
    ...
    (
        ...
        [macros_path] + extra_macros_paths,
        system_sepolicy_path,
        verbose,
    )

    rule_file_paths = resolve_rule_paths(
        [rules_path] + extra_rules_paths,
        system_sepolicy_path,
        verbose,
    )
