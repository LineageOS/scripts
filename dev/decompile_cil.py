...
def decompile_cil():
    ...
    rule_matches: Set[RuleMatch] = match_macros_rules(
        mld,
        macros_name_rules,
        verbose,
    )
    discard_rule_matches(rule_matches, verbose)
    ...
    def process_rules(
        ...
        replace_macro_rules(m, rule_matches, name)
        ...
        merge_typeattribute_rules(m, pt, name)

        replace_perms(m, classmap, source_perms, name)
        replace_ioctls(m, source_ioctls, source_ioctl_defines, name)
        merge_class_sets(m, source_class_sets, name)

    process_rules(mld, public_types, 'private', remove_public=True)

    if split_public_private:
        for public_rule in public_rules:
            public_mld.add(public_rule.hash_values, public_rule)

    process_rules(public_mld, set(), 'public', remove_public=False)
    ...
    output_grouped_rules(grouped_rules, private_output_dir)

    if split_public_private:
        public_output_dir = Path(output_dir, 'public')
        public_output_dir.mkdir(parents=True, exist_ok=True)
        output_grouped_rules(public_grouped_rules, public_output_dir)
