...
def decompile_cil():
    ...
    macro_file_paths, access_vectors_path = resolve_macro_paths(
        [macro_paths],
        system_sepolicy_path,
    )
    extra_macro_file_paths, _ = resolve_macro_paths(extra_macro_paths)
    macro_file_paths += extra_macro_file_paths

    rule_file_paths = resolve_rule_paths([rule_paths] + extra_rule_paths)
    ...
    source_rules = decompile_rules(classmap, freestanding_rule_texts)
    color_print(
    ...
    remove_source_rule_matches(all_rule_matches, source_rules)

    replace_macro_rules(mld, all_rule_matches)

    if platform_decompiled_rules is not None:
        remove_rules(mld, platform_decompiled_rules, 'prebuilt platform')

    remove_rules(mld, source_rules, 'source')
