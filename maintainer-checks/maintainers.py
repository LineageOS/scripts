...
for f in os.listdir(os.path.join(mydir, repo["wiki"] + "/_data/devices")):
    codename = re.sub(r"\.yml", "", f.strip())
    if codename not in codenames:
        wiki_yml_file = os.path.join(mydir, repo["wiki"] + "/_data/devices/" + codename + ".yml")
        with open(wiki_yml_file) as file:
            yml = yaml.load(file)
            if not "discontinued" in yml["channels"]:
