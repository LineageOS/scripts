...
def get_cm_dependencies(repo):
    try:
        stdout = subprocess.run(
            ["git", "ls-remote", "--heads", f"https://:@github.com/LineageOS/{repo}.git"],
    ...
    try:
        cmdeps = requests.get(
            f"https://raw.githubusercontent.com/LineageOS/{repo}/{branch}/lineage.dependencies"
        ).json()
    except:
        cmdeps = []
...
with concurrent.futures.ThreadPoolExecutor(max_workers=args.jobs) as executor:
    ...
    for repo in [x.attrib["name"][10:] for x in elements.findall(".//project")]:
