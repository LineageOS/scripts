...
# Needed for aospremote.
. build/envsetup.sh

TOP="$ANDROID_BUILD_TOP"
MERGEDREPOS="$TOP/merged_repos.txt"
MANIFEST="${TOP}/.repo/manifest.xml"
BRANCH=$(grep "default revision" "$MANIFEST" | sed 's/^ *//g;s/<default revision=\"refs\/heads\///g;s/\"//g')
...
for PROJECTPATH in $PROJECTPATHS; do
  ...
  if [[ -n "$(git status --short)" ]]; then
    ...
    if [[ "$OPERATION" == "merge" ]]; then
     echo -e "merged\t\t$PROJECTPATH" | tee -a "$MERGEDREPOS"
   elif [[ "$OPERATION" == "rebase" ]]; then
     echo -e "rebased\t\t$PROJECTPATH" | tee -a "$MERGEDREPOS"
   fi
  fi
