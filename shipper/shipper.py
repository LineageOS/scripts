...
def fetch_build_targets(croot: str, git_head: str) -> dict:
    build_targets = {}
...
def get_wiki_data(croot: str, codename: str) -> WikiData:
    device_names = []
    maintainers = []
    versions = []
...
def main() -> None:
    ...
    build_targets_before = fetch_build_targets(croot, args.hudson_head_before)
    build_targets_after = fetch_build_targets(croot, 'HEAD')
    ...
    for branch, codenames in new_devices.items():
        ...
        print(textwrap.dedent(f'''\
            ...
            |-------------|------|-------------|------------|'''))
        print('\n'.join(
            sorted([get_wiki_data(croot, x).to_jekyll_table() for x in codenames])
        ))
        print()
