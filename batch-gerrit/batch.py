...
parser.add_argument('--query', '-Q', help='Get changes using query')
...
if not topics and not numbers and args.query is None:
    print('Please specify a change number, topic or query')
    sys.exit(1)
if not steps:
    print('Please specify steps to perform on given changes')
    sys.exit(1)

def build_query(prefixes, topics, numbers, raw_query):
    query = '?q=('
    if raw_query:
        query += raw_query + ')'
        return query
...
print("Fetching info...")
query = build_query([], topics, numbers, raw_query)
...
for s in steps:
    for commit in commits:
        if s == 'abandon':
            ...
            if gerrit.get("/changes/{}".format(commit['_number']))['status'] == 'MERGED':
                print("{} already merged".format(commit['_number']))
                continue
