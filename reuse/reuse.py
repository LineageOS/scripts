...
def fix_files(project_path, extension):
    ...
    for item in path_list:
        ...
        if extension == "*.java" or extension == "*.bp":
...
if __name__ == "__main__":
    ...
    extensions = ["java", "xml", "bp"]
    for ext in extensions:
        fix_files(path, f"*.{ext}")

    license_src_path = str(os.path.join(str(Path.cwd()), "LICENSES"))
    license_dst_path = str(os.path.join(path, "LICENSES"))
    if not os.path.exists(license_dst_path):
        os.mkdir(license_dst_path)
    for lic in USED_LICENSES:
        src_file_name = f"{lic}.txt"
        license_file_src = str(os.path.join(license_src_path, src_file_name))
        license_file_dst = str(os.path.join(license_dst_path, src_file_name))
        if os.path.isfile(license_file_src) and not os.path.isfile(license_file_dst):
            shutil.copy2(license_file_src, license_file_dst)
