...
def fix_files(project_path, extension, used_licenses):
    path_list = Path(project_path).rglob(extension)
    for item in path_list:
        path_in_str = str(item)
        if extension == "*.java" or extension == "*.bp" or extension == "*.proto":
            clean_file(path_in_str, "java", used_licenses)
        if extension == "*.bp" or extension == "*.proto":
            clean_file(path_in_str, "bp", used_licenses)
        if extension == "*.xml":
            clean_file(path_in_str, "xml", used_licenses)
        if extension == "*.py":
            clean_file(path_in_str, "py", used_licenses)
    return


def clean_file(file, comment_type, used_licenses: list):
    ...
    license_type = get_license_type(comment, used_licenses)
    parts = comment.split("\n")
    if len(parts) == 1:
        fh.close()
        return
...
def get_license_type(comment, used_licenses):
    lic = None
...
def main():
    ...
    for ext in extensions:
        fix_files(path, f"*.{ext}", used_licenses)

    # Download all licenses automatically
    os.chdir(path)
    for lic in used_licenses:
        _, code = run_subprocess(["pipx", "run", "reuse", "download", lic], True)
